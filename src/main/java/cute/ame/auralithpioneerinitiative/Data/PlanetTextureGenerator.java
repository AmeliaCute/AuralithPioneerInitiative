...
public final class PlanetTextureGenerator
{
    ...
    public static ResourceLocation getOrGenerate(ProceduralPlanetConfig cfg, String planetId)
    {
        ...
        LOGGER.debug("[Auralith] Generating {} texture for '{}' ({}x{}, seed={})",
                cfg.type(), planetId, res, res, cfg.seed());

        NativeImage image = generate(cfg, res);
        DynamicTexture dynTex = new DynamicTexture(image);

        String safeName = "generated/planet/"
                + planetId.replace(':', '_').replace('/', '_');
        ResourceLocation rl = ResourceLocation.fromNamespaceAndPath(
                "auralithpioneerinitiative", safeName);
    ...
    private static NativeImage generateGasGiant(ProceduralPlanetConfig cfg, int res) {
        ...
        for (int y = 0; y < res; y++) for (int x = 0; x < res; x++) {
            ...
            float t = clamp01(((float)Math.sin((v+turb)*Math.PI*numBands)+1f)*0.5f
                    + fbm(u*5f+turb,v*5f,seed+99L,4,0.5f)*0.30f - 0.15f);
    ...
    private static float valueNoise(float x, float y, long seed) {
        ...
        return lerp(lerp(pseudoRandom(ix,iy,seed),pseudoRandom(ix+1,iy,seed),ux),
                lerp(pseudoRandom(ix,iy+1,seed),pseudoRandom(ix+1,iy+1,seed),ux),uy);
    }
