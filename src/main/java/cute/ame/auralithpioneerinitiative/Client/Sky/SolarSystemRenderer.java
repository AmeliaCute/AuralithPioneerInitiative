...
public final class SolarSystemRenderer
{
    ...
    {
        boolean isSpace = AuralithAPI.getBindingForDimension(level.dimension()).map(AuralithAPI.DimensionBinding::isSpaceDimension).orElse(false);
        if (isSpace) return;
    ...
    private static void addSkyWall(BufferBuilder buf, Matrix4f m, float x1, float z1, float x2, float z2, float r, float g, float b, float d)
    {
    ...
    private void renderPlanetInSky(PoseStack ps, PlanetDefinition planet, float[] theirPos, float apparentSize, float cdx, float cdy, float cdz, long tick, float partialTick)
    {
        ...
        planet.rings().ifPresent(rings -> renderRings(ps, rings, apparentSize, cdx, cdy, cdz, false));
        ...
        planet.rings().ifPresent(rings -> renderRings(ps, rings, apparentSize, cdx, cdy, cdz, true));
    ...
    private void renderFromSpace(PoseStack ps, ClientLevel level, SolarSystemDefinition system, float partialTick) {
        ...
        // TODO: Move the sun somewhere else
        ps.pushPose();
        renderSun(ps, system.sun(), tick, partialTick);
        ps.popPose();
    ...
    private void renderRings(PoseStack ps, RingDefinition rings, float apparentSize)
    {
        ...
        for (int ri = 0; ri < count; ri++)
        {
            ...
            buf.addVertex(m, -r0, 0f, -r1).setColor(cr, cg, cb, ca);
            buf.addVertex(m,  r0, 0f, -r1).setColor(cr, cg, cb, ca);
            buf.addVertex(m,  r0, 0f, -r0).setColor(cr, cg, cb, ca);
            buf.addVertex(m, -r0, 0f, -r0).setColor(cr, cg, cb, ca);
            buf.addVertex(m, -r0, 0f,  r0).setColor(cr, cg, cb, ca);
            buf.addVertex(m,  r0, 0f,  r0).setColor(cr, cg, cb, ca);
            buf.addVertex(m,  r0, 0f,  r1).setColor(cr, cg, cb, ca);
            buf.addVertex(m, -r0, 0f,  r1).setColor(cr, cg, cb, ca);
            buf.addVertex(m, -r1, 0f, -r0).setColor(cr, cg, cb, ca);
            buf.addVertex(m, -r0, 0f, -r0).setColor(cr, cg, cb, ca);
            buf.addVertex(m, -r0, 0f,  r0).setColor(cr, cg, cb, ca);
            buf.addVertex(m, -r1, 0f,  r0).setColor(cr, cg, cb, ca);
            buf.addVertex(m,  r0, 0f, -r0).setColor(cr, cg, cb, ca);
            ...
            buf.addVertex(m,  r1, 0f,  r0).setColor(cr, cg, cb, ca);
            buf.addVertex(m,  r0, 0f,  r0).setColor(cr, cg, cb, ca);
            buf.addVertex(m, -r1, 0f, -r1).setColor(cr, cg, cb, ca);
            buf.addVertex(m, -r0, 0f, -r1).setColor(cr, cg, cb, ca);
            buf.addVertex(m, -r0, 0f, -r0).setColor(cr, cg, cb, ca);
            buf.addVertex(m, -r1, 0f, -r0).setColor(cr, cg, cb, ca);
            buf.addVertex(m,  r0, 0f, -r1).setColor(cr, cg, cb, ca);
    ...
    private void renderRings(PoseStack ps, RingDefinition rings, float apparentSize,
                             float cdx, float cdy, float cdz, boolean front)
    {
        RenderSystem.setShader(GameRenderer::getPositionColorShader);
        RenderSystem.setShaderColor(1f, 1f, 1f, 1f);
        RenderSystem.disableDepthTest();
        ...
        for (int ri = 0; ri < count; ri++)
        {
            ...
            {
                float dot = quadDotX[qi] * cdx + quadDotZ[qi] * cdz;
                if ((dot >= 0) != front) continue;

                for (float[] v : quads[qi])
                    buf.addVertex(m, v[0], v[1], v[2]).setColor(cr, cg, cb, ca);
            }
        }
    ...
    private void renderColoredCube(PoseStack ps, float r, float g, float b, float a) {
        ...
        BufferBuilder buf = tess.begin(VertexFormat.Mode.QUADS, DefaultVertexFormat.POSITION_COLOR);
        Matrix4f m = ps.last().pose();
        for (float[][] face : CUBE_FACES) for (float[] v : face) buf.addVertex(m, v[0] * 0.5f, v[1] * 0.5f, v[2] * 0.5f).setColor(r, g, b, a);
        BufferUploader.drawWithShader(buf.buildOrThrow());
    }
