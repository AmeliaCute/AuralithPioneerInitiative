...
public final class SolarSystemRenderer
{
    ...
    private static final float ORBIT_PLANET_SIZE = 60.0f;
    ...
    {
      skyFogSetup.run();
      Quaternionf skyRot = camera.rotation().conjugate(new Quaternionf());
    ...
    private void renderCelestials(PoseStack ps, ClientLevel level, float partialTick)
    {
      ...
      switch (binding.type())
      {
        ...
        case ORBIT   -> renderFromOrbit(ps, level, sOpt.get(), binding.planetId(), partialTick);
        case SURFACE -> renderFromSurface(ps, level, sOpt.get(), binding.planetId(), partialTick);
      }
    }

    private void renderFromOrbit(PoseStack ps, ClientLevel level, SolarSystemDefinition system, ResourceLocation currentPlanetId, float partialTick)
    {
      ...
      for (PlanetDefinition moon : current.moons()) renderMoonFromOrbit(ps, moon, tick, partialTick);
      ...
      for (PlanetDefinition planet : system.planets())
      {
        ...
        renderPlanetInSky(ps, planet, new float[]{dx, dy, dz}, apparentSize, cdx, cdy, cdz, tick, partialTick);
      }
    }
    ...
    private void renderFromSurface(PoseStack ps, ClientLevel level, SolarSystemDefinition system, ResourceLocation currentPlanetId, float partialTick)
    {
      ...
      renderSun(ps, system.sun(), tick, partialTick, 0f, 1f, 0f);
    ...
    private void renderFromSpace(PoseStack ps, ClientLevel level, SolarSystemDefinition system, float partialTick)
    {
      ...
      renderSun(ps, system.sun(), tick, partialTick, sunNX, sunNY, sunNZ);
      ...
      for (PlanetDefinition planet : system.planets())
      {
        ...
        for (PlanetDefinition moon : planet.moons()) renderMoon(ps, moon, apparentSize, tick, partialTick);

        ps.popPose();
      }
    }

    private void renderPlanetInSky(PoseStack ps, PlanetDefinition planet, float[] dirPos, float apparentSize, float cdx, float cdy, float cdz, long tick, float partialTick)
    {
    ...
    private void renderSun(PoseStack poseStack, SunDefinition sun, long tick, float partialTick, float nx, float ny, float nz)
    {
      poseStack.pushPose();
      poseStack.translate(nx * SKY_RADIUS, ny * SKY_RADIUS, nz * SKY_RADIUS);
      poseAxialRotation(poseStack, sun.axialRotationSpeed(), tick, partialTick);
